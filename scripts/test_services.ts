...
const runSelfDiagnostic = async () => {
  ...
  const parsedProduct = ProductValidations.createProductSchema.safeParse({ body: invalidProductBody });
  if (!parsedProduct.success) {
    console.log(colors.green('✔ Zod successfully caught invalid product input payload:'));
    parsedProduct.error.errors.forEach(err => {
      console.log(colors.gray(`   ↳ Path: "${err.path.join('.')}" | Message: "${err.message}"`));
    });
  } else {
  ...
  const parsedBid = AuctionValidations.placeBidSchema.safeParse({ body: invalidBidBody });
  ...
  try {
    ...
    const mockUid = 999;
    const tokenPayload = await AuctionServices.generateAgoraToken(mockChannel, mockUid, 'publisher');
    
    console.log(colors.green('✔ Agora Token generation successful:'));
    console.log(colors.gray(`   ↳ Channel Name: "${tokenPayload.channelName}"`));
    console.log(colors.gray(`   ↳ RTC Token: "${tokenPayload.token.substring(0, 30)}..."`));
  ...
  if (!dbUrl) {
    console.log(colors.red('✖ DATABASE_URL is missing in environment variables. DB service test skipped.'));
    console.log(colors.cyan('\n=================================================='));
    return;
  }
  ...
  try {
    ...
    // Diagnostic complete
    console.log(colors.green('✔ All business logical engines are compiled and verified!'));
  } catch (err: any) {

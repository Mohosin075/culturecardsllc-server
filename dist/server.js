...
async function main() {
  try {
    ...
    server = app_1.default.listen(port, '0.0.0.0', () => {
      ...
      logger_1.logger.info(
        colors_1.default.cyan(`   - Local:    http://localhost:${port}`),
      )
      ...
      if (config_1.default.ip_address) {
        logger_1.logger.info(
          colors_1.default.green(
            `   - Requested IP: http://${config_1.default.ip_address}:${port}`,
          ),
        )
      }
    })
    ...
    logger_1.logger.info(
      colors_1.default.green('🍁 Socket.IO initialized successfully'),
    )
  } catch (error) {

    ...
    const tocHtml = tocItems.length > 0
      ...
      <details class="mobile-toc">
        ...
        <ul class="mobile-toc-list">
          ${tocItems
            .map(item => `<li><a href="#${item.id}" onclick="this.closest('details').removeAttribute('open')">${item.title}</a></li>`)
            .join('\n')}
        </ul>
      </details>`
        : '';
...
app.get('/privacy-policy', async (req, res) => {
    ...
    res.send(renderLegalHtml('Privacy Policy', '<p>The Privacy Policy is currently being updated. Please check back shortly.</p>', 'privacy-policy'));
});
const handleTermsAndConditions = async (req, res) => {
    ...
    res.send(renderLegalHtml('Terms & Conditions', '<p>The Terms & Conditions are currently being updated. Please check back shortly.</p>', 'terms-and-condition'));
};

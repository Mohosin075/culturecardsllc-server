...
const toggleFavouriteZodSchema = zod_1.z.object({
  body: zod_1.z
    .object({
      favouriteType: zod_1.z.enum(
        Object.values(favourite_interface_1.FavouriteType),
      ),
    ...
    .refine(
      data => {
        if (
          data.favouriteType === favourite_interface_1.FavouriteType.SERVICE &&
          !data.service
        )
          return false
        if (
          data.favouriteType === favourite_interface_1.FavouriteType.PROVIDER &&
          !data.provider
        )
          return false
        return true

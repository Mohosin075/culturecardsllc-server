...
const updateProfile = async (user, payload) => {
  ...
  const updatedProfile = await user_model_1.User.findOneAndUpdate(
    { _id: user.userId, status: { $nin: [user_1.USER_STATUS.DELETED] } },
    {
      $set: payload,
    },
    { new: true },
...
const createAdmin = async () => {
  var _a, _b
  const email =
    (_a = config_1.default.super_admin.email) === null || _a === void 0
      ? void 0
      : _a.toLowerCase().trim()
  const name =
    (_b = config_1.default.super_admin.name) === null || _b === void 0
      ? void 0
      : _b.trim()
...
const deactivateProfile = async (userId, password) => {
  ...
  const deactivatedUser = await user_model_1.User.findOneAndUpdate(
    { _id: userId, status: { $nin: [user_1.USER_STATUS.DELETED] } },
    { $set: { status: user_1.USER_STATUS.INACTIVE } },
    { new: true },
  )
  if (!deactivatedUser) {
    throw new ApiError_1.default(
...
const switchRole = async (user, role) => {
  ...
  if (
    role === user_1.USER_ROLES.PROFESSIONAL &&

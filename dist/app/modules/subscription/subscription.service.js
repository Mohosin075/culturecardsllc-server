...
class SubscriptionService {
  ...
  async getPlanById(planId) {
    try {
      const plan =
        await subscription_plan_model_1.SubscriptionPlan.findById(planId)
      if (!plan) {
        throw new ApiError_1.default(
      ...
      if (error instanceof ApiError_1.default) throw error
  ...
  async checkTrialEligibility(userId) {
    try {
      const existingSubscription =
        await subscription_model_1.Subscription.findOne({
  ...
  async createSubscription(userId, request) {
    try {
      ...
      const existingSubscription =
        await subscription_model_1.Subscription.findActiveByUserId(userId)
      ...
      const existingCustomer = await subscription_model_1.Subscription.findOne({
        userId,
      }).select('stripeCustomerId')
      if (
        existingCustomer === null || existingCustomer === void 0
          ? void 0
        ...
        const stripeCustomer =
          await stripe_service_1.stripeService.createCustomer(
            user.email,
      ...
      if (request.paymentMethodId) {
        await stripe_service_1.stripeService.attachPaymentMethod(
          request.paymentMethodId,
          stripeCustomerId,
        )
        await stripe_service_1.stripeService.setDefaultPaymentMethod(
          stripeCustomerId,
          request.paymentMethodId,
        )
      }
      ...
      const stripeSubscription =
        await stripe_service_1.stripeService.createSubscription({
      ...
      // Send welcome email
      await email_notification_service_1.emailNotificationService.sendSubscriptionWelcomeEmail(
        subscription,
        plan,
      ...
      if (!clientSecret && stripeSubscription.pending_setup_intent) {
        if (typeof stripeSubscription.pending_setup_intent === 'object') {
          clientSecret =
            stripeSubscription.pending_setup_intent.client_secret || undefined
  ...
  async getAllSubscriptions(query) {
    try {
      const subscriptionQuery = new QueryBuilder_1.default(
        subscription_model_1.Subscription.find().populate(['userId', 'planId']),
        query,
      )
        .filter()
        .sort()
  ...
  async updateSubscription(userId, subscriptionId, request) {
    try {
      ...
      if (request.planId) {
        ...
        const stripeSubscription =
          await stripe_service_1.stripeService.getSubscription(
            subscription.stripeSubscriptionId,
          )
        const subscriptionItemId = stripeSubscription.items.data[0].id
        // Update Stripe subscription with correct item ID
        await stripe_service_1.stripeService.updateSubscription(
          subscription.stripeSubscriptionId,
          {
            items: [
              {
        ...
        const { emailNotificationService } = await Promise.resolve().then(() =>
          __importStar(require('./email-notification.service')),
        )
      ...
      const updatedSubscription =
        await subscription_model_1.Subscription.findByIdAndUpdate(
          subscriptionId,
          updateParams,
          { new: true },
        ).populate(['planId'])
  ...
  async reactivateSubscription(userId, subscriptionId) {
    try {
      ...
      await stripe_service_1.stripeService.reactivateSubscription(
        subscription.stripeSubscriptionId,
      )
      // Update local subscription
      const updatedSubscription =
      ...
      const plan =
        updatedSubscription === null || updatedSubscription === void 0
          ? void 0
  ...
  async createCheckoutSession(userId, planId, successUrl, cancelUrl) {
    try {
      ...
      const session =
        await stripe_service_1.stripeService.createCheckoutSession({
          customerId: stripeCustomerId,
          priceId: plan.stripePriceId,
          successUrl,
          cancelUrl,
  ...
  async updateSubscriptionPlan(planId, updateData) {
    try {
      ...
      if (updateData.name || updateData.description) {
        await stripe_service_1.stripeService.updateProduct(
      ...
      const updatedPlan =
        await subscription_plan_model_1.SubscriptionPlan.findByIdAndUpdate(
          planId,
          updateData,
          { new: true },
        )
  ...
  async retryFailedPayment(subscriptionId) {
    try {
      const subscription =
        await subscription_model_1.Subscription.findById(subscriptionId)
      if (!subscription) {
        throw new ApiError_1.default(
          http_status_codes_1.StatusCodes.NOT_FOUND,
          'Subscription not found',
      ...
      const stripeSubscription =
        await stripe_service_1.stripeService.getSubscriptionExpanded(
          subscription.stripeSubscriptionId,
        )
      if (
        stripeSubscription.latest_invoice &&
  ...
  async pauseSubscription(userId, subscriptionId) {
    try {
      ...
      // Pause in Stripe
      await stripe_service_1.stripeService.pauseSubscription(
        subscription.stripeSubscriptionId,
      )
      // Update local subscription
      const updatedSubscription =
  ...
  async resumeSubscription(userId, subscriptionId) {
    try {
      ...
      // Resume in Stripe
      await stripe_service_1.stripeService.resumeSubscription(
        subscription.stripeSubscriptionId,
      )
      // Update local subscription
      const updatedSubscription =

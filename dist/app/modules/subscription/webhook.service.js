...
class WebhookService {
  ...
  async handleSubscriptionCreated(stripeSubscription, eventId) {
    ...
    try {
      const userId =
        (_a = stripeSubscription.metadata) === null || _a === void 0
          ? void 0
          : _a.userId
      if (!userId) {
  ...
  async handleSubscriptionUpdated(stripeSubscription, eventId) {
    try {
      ...
      if (subscription.stripePriceId !== newPriceId) {
        const newPlan =
          await subscription_plan_model_1.SubscriptionPlan.findOne({
  ...
  async handleSubscriptionDeleted(stripeSubscription, eventId) {
    try {
      ...
      const plan = await subscription_plan_model_1.SubscriptionPlan.findById(
        subscription.planId,
      )
      if (plan) {
  ...
  async handleCheckoutCompleted(session, eventId) {
    ...
    try {
      ...
      const userId =
        (_a = session.metadata) === null || _a === void 0 ? void 0 : _a.userId
      if (!userId) {
  ...
  async handlePaymentMethodAttached(paymentMethod, _eventId) {
    try {
  ...
  async handleCustomerUpdated(stripeCustomer, _eventId) {
    try {
  ...
  async handleInvoiceCreated(invoice, _eventId) {
    try {
      const invoiceWithSubscription = invoice
  ...
  async handleSetupIntentSucceeded(setupIntent, _eventId) {
    try {
  ...
  async handlePaymentMethodUpdated(paymentMethod, _eventId) {
    try {
  ...
  verifyWebhookSignature(payload, signature) {
    try {
      return stripe_service_1.stripeService.constructWebhookEvent(
        payload,
        signature,
      )
    } catch (error) {

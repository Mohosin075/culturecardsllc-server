...
const getAvailablePlans = (0, catchAsync_1.default)(async (req, res) => {
  ...
  const plans =
    await subscription_service_1.subscriptionService.getAvailablePlans(userType)
...
const getPlanById = (0, catchAsync_1.default)(async (req, res) => {
  ...
  const plan =
    await subscription_service_1.subscriptionService.getPlanById(planId)
  ;(0, sendResponse_1.default)(res, {
    statusCode: http_status_codes_1.StatusCodes.OK,
    success: true,
...
const checkTrialEligibility = (0, catchAsync_1.default)(async (req, res) => {
  ...
  const userId =
    req.params.userId ||
    ((_a = user.authId) === null || _a === void 0 ? void 0 : _a.toString())
  const trialInfo =
    await subscription_service_1.subscriptionService.checkTrialEligibility(
      userId,
    )
  ;(0, sendResponse_1.default)(res, {
    statusCode: http_status_codes_1.StatusCodes.OK,
...
const getUserSubscription = (0, catchAsync_1.default)(async (req, res) => {
  ...
  const userId = user.authId.toString()
  const subscription =
    await subscription_service_1.subscriptionService.getUserSubscription(userId)
  ;(0, sendResponse_1.default)(res, {
    statusCode: http_status_codes_1.StatusCodes.OK,
    success: true,
...
const getSubscriptionStatus = (0, catchAsync_1.default)(async (req, res) => {
  ...
  const userId = user.authId.toString()
  const status =
    await subscription_service_1.subscriptionService.getSubscriptionStatus(
      userId,
    )
  ;(0, sendResponse_1.default)(res, {
    statusCode: http_status_codes_1.StatusCodes.OK,
...
const createCheckoutSession = (0, catchAsync_1.default)(async (req, res) => {
  ...
  const session =
    await subscription_service_1.subscriptionService.createCheckoutSession(
      userId,
      planId,
      successUrl,
      cancelUrl,
...
const handleWebhook = (0, catchAsync_1.default)(async (req, res) => {
  ...
  const event = webhook_service_1.webhookService.verifyWebhookSignature(
    payload,
    signature,
  )
...
const createSubscriptionPlan = (0, catchAsync_1.default)(async (req, res) => {
  const plan =
    await subscription_service_1.subscriptionService.createSubscriptionPlan(
      req.body,
    )
  ;(0, sendResponse_1.default)(res, {
    statusCode: http_status_codes_1.StatusCodes.CREATED,
...
const updateSubscriptionPlan = (0, catchAsync_1.default)(async (req, res) => {
  const { planId } = req.params
  const plan =
    await subscription_service_1.subscriptionService.updateSubscriptionPlan(
      planId,
      req.body,
    )
  ;(0, sendResponse_1.default)(res, {
...
const getAllSubscriptions = (0, catchAsync_1.default)(async (req, res) => {
  const result =
    await subscription_service_1.subscriptionService.getAllSubscriptions(
      req.query,
    )
  ;(0, sendResponse_1.default)(res, {
    statusCode: http_status_codes_1.StatusCodes.OK,
...
const getSubscriptionAnalytics = (0, catchAsync_1.default)(async (req, res) => {
  ...
  const analytics =
    await subscription_service_1.subscriptionService.getSubscriptionAnalytics(
      filters,
    )
  ;(0, sendResponse_1.default)(res, {
    statusCode: http_status_codes_1.StatusCodes.OK,
...
const reactivateSubscription = (0, catchAsync_1.default)(async (req, res) => {
  ...
  const { subscriptionId } = req.params
  const subscription =
    await subscription_service_1.subscriptionService.reactivateSubscription(
      userId,
      subscriptionId,
    )
  ;(0, sendResponse_1.default)(res, {
...
const retryFailedPayment = (0, catchAsync_1.default)(async (req, res) => {
  const { subscriptionId } = req.params
  await subscription_service_1.subscriptionService.retryFailedPayment(
    subscriptionId,
  )
  ;(0, sendResponse_1.default)(res, {
    statusCode: http_status_codes_1.StatusCodes.OK,
...
const pauseSubscription = (0, catchAsync_1.default)(async (req, res) => {
  ...
  const { subscriptionId } = req.params
  const subscription =
    await subscription_service_1.subscriptionService.pauseSubscription(
      userId,
      subscriptionId,
    )
  ;(0, sendResponse_1.default)(res, {
...
const resumeSubscription = (0, catchAsync_1.default)(async (req, res) => {
  ...
  const { subscriptionId } = req.params
  const subscription =
    await subscription_service_1.subscriptionService.resumeSubscription(
      userId,
      subscriptionId,
    )
  ;(0, sendResponse_1.default)(res, {
...
const getUsageData = (0, catchAsync_1.default)(async (req, res) => {
  const user = req.user
  const userId = user.authId.toString()
  const { usageTrackingService } = await Promise.resolve().then(() =>
    __importStar(require('./usage-tracking.service')),
  )
...
const createBillingPortal = (0, catchAsync_1.default)(async (req, res) => {
  ...
  const portalSession =
    await subscription_service_1.subscriptionService.createBillingPortalSession(
      userId,
      returnUrl,
    )
  ;(0, sendResponse_1.default)(res, {

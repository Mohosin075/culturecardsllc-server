...
const createOrderSchema = zod_1.z.object({
    body: zod_1.z.object({
        buyerId: zod_1.z.string({ required_error: 'Buyer ID is required' }).regex(/^[0-9a-fA-F]{24}$/, 'Invalid Buyer ID format'),
        ...
        purchaseType: zod_1.z.enum(['auction_win', 'buy_now', 'trade_swap'], { required_error: 'Purchase type is required' }),
        amountDetails: zod_1.z.object({
            itemSubtotal: zod_1.z.number({ required_error: 'Subtotal is required' }).nonnegative(),
            ...
            totalPaid: zod_1.z.number({ required_error: 'Total paid amount is required' }).nonnegative()
...
const updateOrderJourneySchema = zod_1.z.object({
    body: zod_1.z.object({
        status: zod_1.z.string({ required_error: 'Status label is required' }).min(2, 'Status must be at least 2 characters'),
        description: zod_1.z.string({ required_error: 'Description is required' }).min(3, 'Description must be at least 3 characters'),
        location: zod_1.z.string().optional(),
        deliveryStatus: zod_1.z.enum(['pending', 'shipped', 'delivered', 'cancelled']).optional()
    })
});

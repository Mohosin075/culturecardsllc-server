...
const getOrdersForUser = (0, catchAsync_1.default)(async (req, res) => {
  ...
  const result = await order_service_1.OrderServices.getOrdersForUser(
    userId,
    role,
  )
  ;(0, sendResponse_1.default)(res, {
...
const updateOrderJourney = (0, catchAsync_1.default)(async (req, res) => {
  ...
  if (!status || !description) {
    throw new ApiError_1.default(
      http_status_codes_1.StatusCodes.BAD_REQUEST,
      'status and description are required for journey updates.',
    )
  }
  const result = await order_service_1.OrderServices.updateOrderJourney(
    req.params.id,

...
const verifyCheckoutSession = async sessionId => {
  try {
    ...
    const session = await stripe_1.default.checkout.sessions.retrieve(
      sessionId,
      {
...
const createPaymentIntent = async (user, payload) => {
  try {
    ...
    const paymentIntent =
      await stripe_1.default.paymentIntents.create(intentParams)
...
const createEphemeralKey = async (user, apiVersion = '2025-05-28.basil') => {
  try {
    ...
    const ephemeralKey = await stripe_1.default.ephemeralKeys.create(
      { customer: customerId },
...
const getAllPayments = async (user, filterables, pagination) => {
  ...
  if (
    user.activeRole === user_1.USER_ROLES.USER ||
...
const getSinglePayment = async id => {
  ...
  const result = await payment_model_1.Payment.findById(id).populate(
    'userId',
    'name email',
  )
  if (!result) {
...
const updatePayment = async (id, payload) => {
  ...
  const result = await payment_model_1.Payment.findByIdAndUpdate(
...
const refundPayment = async (id, reason) => {
  ...
  try {
    ...
    const checkoutSessionId =
      (_a = payment.metadata) === null || _a === void 0
        ? void 0
        : _a.checkoutSessionId
    // For checkout payments, paymentIntentId may hold session ID in older records.
    if (checkoutSessionId) {
      const checkoutSession =
        await stripe_1.default.checkout.sessions.retrieve(checkoutSessionId)
    ...
    let result = null
    try {
      result = await payment_model_1.Payment.findByIdAndUpdate(
        id,
        {
          status: 'refunded',
...
const generateInvoice = async id => {
  ...
  if (
    ...
    try {
      const pi = await stripe_1.default.paymentIntents.retrieve(
        payment.paymentIntentId,
      )
      if (pi.latest_charge) {
...
const getMyPaymentMethods = async user => {
  ...
  const customer = await stripe_1.default.customers.retrieve(
    userData.stripeCustomerId,
  )
  ...
  return paymentMethods.data.map(pm => {
    ...
    return {
      ...
      expMonth:
        (_c = pm.card) === null || _c === void 0 ? void 0 : _c.exp_month,

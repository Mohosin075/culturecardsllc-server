  ...
  io.on('connection', socket => {
      ...
      if (userId) {
        ...
        console.log(
          colors_1.default.green(
            `User ${socket.id} joined notification room: ${userId}`,
          ),
        )
    ...
    socket.on('join-stream', async data => {
      ...
      if (streamId) {
        ...
        console.log(
          colors_1.default.green(
            `User ${socket.id} joined stream room: ${streamId}`,
          ),
        )
        // Increment Mongoose stream viewer count
        const stream = await auction_model_1.LiveStream.findByIdAndUpdate(
          streamId,
          { $inc: { viewersCount: 1 } },
          { new: true },
        )
      ...
      if (streamId) {
        ...
        console.log(
          colors_1.default.yellow(
            `User ${socket.id} left stream room: ${streamId}`,
          ),
        )
        const stream = await auction_model_1.LiveStream.findByIdAndUpdate(
          streamId,
    ...
    socket.on('place-bid', async data => {
      ...
      try {
        ...
        const updatedAuction =
          await auction_service_1.AuctionServices.placeBidSecure(
            auctionItemId,
            bidderId,
            bidAmount,
          )
        // Fetch bidder profile for display name in feed
        const bidderInfo = await user_model_1.User.findById(bidderId).select(
    ...
    socket.on('stream-chat', async data => {
      ...
      if (streamId && userId && message) {
        const userInfo = await user_model_1.User.findById(userId).select(
          'name fullName email image photo',
        )

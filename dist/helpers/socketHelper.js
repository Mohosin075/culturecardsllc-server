...
const pickWeightedPrize = () => {
    ...
    for (const prize of SPIN_WHEEL_PRIZES) {
        ...
        if (randomValue < cumulativeWeight) {
            ...
            const degreeIndex = Math.floor(Math.random() * (prize.degreeMax - prize.degreeMin + 1) + prize.degreeMin);
        ...
        socket.on('leave-stream', async (streamId) => {
            if (streamId) {
                ...
                if (stream && stream.viewersCount < 0) {
                    await auction_model_1.LiveStream.findByIdAndUpdate(streamId, { $set: { viewersCount: 0 } });
                }
                io.to(streamRoom).emit('viewer-count-update', {
                    streamId,

...
const SPIN_WHEEL_PRIZES = [
  { name: '10% Shop Coupon', rarity: 'Common', weight: 65, degreeMin: 0, degreeMax: 90 },
  { name: 'Free Mystery Sticker Pack', rarity: 'Rare', weight: 25, degreeMin: 91, degreeMax: 210 },
  { name: 'Rare Card Sleeves', rarity: 'Epic', weight: 9, degreeMin: 211, degreeMax: 330 },
  { name: 'Vintage Booster Pack (Legendary Drop)', rarity: 'Legendary', weight: 1, degreeMin: 331, degreeMax: 359 }
];
  ...
  for (const prize of SPIN_WHEEL_PRIZES) {
    ...
    if (randomValue < cumulativeWeight) {
      ...
      const degreeIndex = Math.floor(
        Math.random() * (prize.degreeMax - prize.degreeMin + 1) + prize.degreeMin
      );
  ...
  io.on('connection', (socket: Socket) => {
    ...
    socket.on('join-stream', async (data: { streamId: string; userId?: string }) => {
      ...
      if (streamId) {
        ...
        console.log(colors.green(`User ${socket.id} joined stream room: ${streamId}`));
    ...
    socket.on('leave-stream', async (streamId: string) => {
      if (streamId) {
        ...
        console.log(colors.yellow(`User ${socket.id} left stream room: ${streamId}`));
        ...
        if (stream && stream.viewersCount < 0) {
          await LiveStream.findByIdAndUpdate(streamId, { $set: { viewersCount: 0 } });
        }
    ...
    socket.on('place-bid', async (data: {
      ...
      try {
        if (!auctionItemId || !bidAmount || !bidderId) {
          socket.emit('bid-error', { message: 'Missing required bid parameters.' });
          return;
        }
        ...
        const bidderInfo = await User.findById(bidderId).select('name fullName email image photo');
        ...
        console.log(colors.green(`Bid placed successfully: $${bidAmount} by User:${bidderId}`));
    ...
    socket.on('stream-chat', async (data: {
      ...
      if (streamId && userId && message) {
        const userInfo = await User.findById(userId).select('name fullName email image photo');
    ...
    socket.on('trigger-spin', async (data: {
      ...
      try {
        ...
        if (stream.sellerId.toString() !== sellerId) {
          socket.emit('spin-error', { message: 'Unauthorized: Only the stream host can trigger the Spin Wheel.' });
          return;
        }
        ...
        console.log(colors.magenta(`Spin Wheel triggered. Prize: ${outcome.name} (${outcome.rarity})`));
      } catch (err: any) {
        socket.emit('spin-error', { message: err.message || 'Spin wheel trigger failed.' });
      }
    });

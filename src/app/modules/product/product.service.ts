...
const getAllProducts = async (filters: {
  ...
  return await Product.find(query).populate('sellerId', 'name fullName email image photo');
};
...
const getProductById = async (id: string): Promise<IProduct> => {
  ...
  const result = await Product.findById(id).populate('sellerId', 'name fullName email image photo stripeCustomerId');
  if (!result) {

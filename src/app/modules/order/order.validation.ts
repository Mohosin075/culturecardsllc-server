...
const createOrderSchema = z.object({
  body: z.object({
    buyerId: z.string({ required_error: 'Buyer ID is required' }).regex(/^[0-9a-fA-F]{24}$/, 'Invalid Buyer ID format'),
    ...
    purchaseType: z.enum(['auction_win', 'buy_now', 'trade_swap'], { required_error: 'Purchase type is required' }),
    amountDetails: z.object({
      itemSubtotal: z.number({ required_error: 'Subtotal is required' }).nonnegative(),
      ...
      totalPaid: z.number({ required_error: 'Total paid amount is required' }).nonnegative()
...
const updateOrderJourneySchema = z.object({
  body: z.object({
    status: z.string({ required_error: 'Status label is required' }).min(2, 'Status must be at least 2 characters'),

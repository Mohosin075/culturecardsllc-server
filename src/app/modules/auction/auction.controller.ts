...
const generateAgoraToken = catchAsync(async (req: Request, res: Response) => {
  ...
  if (!channelName) {
    throw new ApiError(StatusCodes.BAD_REQUEST, 'channelName is required as query parameter.');
  }

  const result = await AuctionServices.generateAgoraToken(channelName, uid, role);
  sendResponse(res, {
...
const placeBidSecure = catchAsync(async (req: Request, res: Response) => {
  ...
  const result = await AuctionServices.placeBidSecure(auctionItemId, bidderId, bidAmount);
  sendResponse(res, {
    statusCode: StatusCodes.OK,
    success: true,
